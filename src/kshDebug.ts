	...
	protected launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments): void {
		...
		if (process.platform === "darwin" && args.pathPkill === "pkill") {
			...
			if (pathPkill === "/usr/local/bin/pkill") {
				const url = "https://github.com/rogalmic/vscode-ksh-debug/wiki/macOS:-avoid-use-of--usr-local-bin-pkill"
		...
		this.proxyProcess.stdin.write(`examine Debug environment: ksh_ver=$BASH_VERSION, kshdb_ver=$_Dbg_release, program=$0, args=$*\nprint "$PPID"\nhandle INT stop\nprint '${KshDebugSession.END_MARKER}'\n`);
